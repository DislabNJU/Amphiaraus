BUILD_DIR="/home/lxb/workspace/hadoop-2.6.2-src/hadoop-hdfs-project/hadoop-hdfs/target"
                      TAR='tar cf -'
                      UNTAR='tar xfBp -'
                      LIB_DIR="${BUILD_DIR}/native/target/usr/local/lib"
                      if [ -d ${LIB_DIR} ] ; then
                        TARGET_DIR="${BUILD_DIR}/hadoop-hdfs-2.6.2/lib/native"
                        mkdir -p ${TARGET_DIR}
                        cd ${LIB_DIR}
                      ...
                      if [ -d ${BIN_DIR} ] ; then
                        TARGET_BIN_DIR="${BUILD_DIR}/hadoop-hdfs-2.6.2/bin"
