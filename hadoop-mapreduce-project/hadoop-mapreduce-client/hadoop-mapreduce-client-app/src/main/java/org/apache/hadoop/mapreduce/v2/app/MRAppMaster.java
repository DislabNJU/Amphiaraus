...
public class MRAppMaster extends CompositeService {
  ...
  private ContainerStatusesMonitoringThread containerStatusesMonitoringThread;
  private boolean containerLauncherStartFinish;
  ...
  public MRAppMaster(ApplicationAttemptId applicationAttemptId,
    ...
    this.containerStatusesMonitoringThread = new ContainerStatusesMonitoringThread();  //added by lxb;
    this.containerLauncherStartFinish = false;
  ...
  private final class ContainerLauncherRouter extends AbstractService
    ...
    protected void serviceStart() throws Exception {
      ...
      super.serviceStart();
      containerLauncherStartFinish = true;
    }
    ...
    //added by lxb
    @Override
    public List<ContainerStatus> getContainerStatuses() throws YarnException,
    IOException {
    	LOG.info("getContainerStatuses in ContainerLauncherRouter.");
    	return this.containerLauncher.getContainerStatuses();
    }
  }
  ...
  //added by lxb
    this.containerStatusesMonitoringThread.start();  
  ...
  @Override
  protected void serviceStop() throws Exception {
      this.containerStatusesMonitoringThread.interrupt();
      try {
        this.containerStatusesMonitoringThread.join();
      } catch (InterruptedException e) {
        ;
      }
  ...
  //added by lxb
  private class ContainerStatusesMonitoringThread extends Thread{
	    public ContainerStatusesMonitoringThread() {
	        super("ContainerStatusesMonitoringThread");
	      }
	    
	    @Override
	    public void run(){
	        while(!containerLauncherStartFinish){
	        	try {
					Thread.sleep(10);
				} catch (InterruptedException e) {
	    	...
	    	while(true){
	    		LOG.info("ContainerStatusesMonitoringThread tring to show containerDetails." );
	    		if(job.isUber()){
	    			//TODO
	    		}else{
	    			LOG.info("Not Uber mode." );
	    			//ContainerLauncherImpl containerLauncherImpl = (ContainerLauncherImpl) (((ContainerLauncherRouter)containerLauncher).getContainerLauncher());
	    			try {
						List<ContainerStatus> containerStatuses = containerLauncher.getContainerStatuses();
					    for(Iterator<ContainerStatus> it = containerStatuses.iterator(); it.hasNext();){
	            ...
	            try {
	                ...
	                LOG.warn( "ContainerStatusesMonitoringThread is interrupted. Exiting.");
	                break;
	              }
	    	}
	    }
